...
const HOLD_EXERCISES = [
  "Side Plank", "Forward Head Posture Hold", "Single Leg Glute Bridge",
  "Lying Floor Row Hold", "Holding Squat", "Hip Flexor Stretch",
  "Bent Arm Chest Stretch", "Sitting Neck Flexion Stretch", "Kneeling Thoracic Spine Extension",
];

const PER_SIDE_EXERCISES = [
  "Side Plank", "Hip Flexor Stretch", "Bent Arm Chest Stretch", "Single Leg Glute Bridge",
];

function parseDosage(exercise) {
  const match = (exercise.dosage || "").match(/(\d+)\s*x\s*(\d+)s/);
  if (match) return { sets: parseInt(match[1], 10), secsPerSet: parseInt(match[2], 10) };
  return { sets: 1, secsPerSet: exercise.durationSecs || 30 };
}
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          <AnimatePresence mode="wait">
            ...
            >
              {done ? `${TOTAL_BREATHS} breaths complete`
                : running ? (isInhale ? "Inhale through your nose" : "Exhale slowly")
                : "Press play to begin"}
            </motion.p>
          </AnimatePresence>
...
function HoldTimer({ exercise }) {
  ...
  useEffect(() => { setSecondsLeft(secsPerSet); setRunning(false); setStageDone(false); }, [stage, secsPerSet]);

  useEffect(() => {
    if (!running) { clearInterval(intervalRef.current); return; }
    intervalRef.current = setInterval(() => {
      setSecondsLeft((s) => {
        if (s <= 1) { clearInterval(intervalRef.current); setRunning(false); setStageDone(true); return 0; }
        return s - 1;
      });
    }, 1000);
    return () => clearInterval(intervalRef.current);
  ...
  const handleNextStage = () => {
    const next = stage + 1;
    if (next >= totalStages) setAllDone(true);
    else setStage(next);
  };
  ...
  function getStageLabel() {
    ...
    if (stageDone) {
      if (isPerSide) {
        ...
        return stage + 1 < totalStages ? `${side} side done — switch to ${next}` : "Both sides complete!";
      }
      const setNum = stage + 1;
      return setNum < sets ? `Set ${setNum} done — rest, then start set ${setNum + 1}` : "All sets complete!";
    }
    if (isPerSide) {
      const side = stage % 2 === 0 ? "Left" : "Right";
    ...
    const setNum = stage + 1;
    return running ? `Set ${setNum} of ${sets} — hold steady`
      : sets === 1 ? "Press play to start" : `Set ${setNum} of ${sets} — press play`;
  }
      ...
      <div className="flex items-center gap-4">
        <div className="relative w-20 h-20 shrink-0">
          <svg className="w-20 h-20 -rotate-90" viewBox="0 0 100 100">
            <circle cx="50" cy="50" r={R} fill="none" stroke="hsl(var(--secondary))" strokeWidth="7" />
            <motion.circle
              cx="50" cy="50" r={R} fill="none" stroke={style.ring}
              strokeWidth="7" strokeLinecap="round" strokeDasharray={circumference}
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            {allDone ? <CheckCircle2 className={`w-6 h-6 ${style.text}`} />
              : stageDone ? <RefreshCw className={`w-5 h-5 ${style.text}`} />
              : (
                <>
        ...
        <div className="flex-1 min-w-0">
          {/* Dots */}
          <div className={`flex items-center gap-1 mb-2 ${style.text}`}>
            {Array.from({ length: totalStages }).map((_, i) => (
              <div key={i} className={`rounded-full transition-all duration-300 ${
                i === stage && !allDone ? "w-4 h-2 bg-current opacity-100"
                  : i < stage || allDone ? "w-2 h-2 bg-current opacity-40"
    ...
    <div className="rounded-3xl border border-border bg-card p-3">
      <div className="flex items-center justify-center gap-1.5 mb-3">
        {Array.from({ length: totalExercises }).map((_, i) => (
          <div key={i} className={`rounded-full transition-all duration-300 ${
            i === exerciseIndex ? "w-5 h-2 bg-primary"
              : i < exerciseIndex ? "w-2 h-2 bg-primary/40"
              : "w-2 h-2 bg-border"
          }`} />
        ))}
      </div>
      <div className="flex gap-2">
        <Button
          variant="outline" onClick={onPrev} disabled={isFirst}
...
export default function ExerciseTimer({
  ...
  return (
    ...
    >
      <div className="max-w-lg mx-auto flex flex-col px-4 pt-4 pb-16" style={{ minHeight: "100%" }}>
        ...
        {/* ── Unified content order for ALL exercises ── */}
        <div className="space-y-3 pb-2">

          {/* 1. Timer (breathing or hold) */}
          {isBreathing && <BreathingTimer />}
          {isHold && <HoldTimer exercise={exercise} />}

          {/* 2. Nav — same position for every exercise */}
          <MidNav
