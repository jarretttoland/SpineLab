...
import { Flame, Shield, Zap, TrendingUp, ChevronRight } from "lucide-react";
...
// ── What moved — plain English explanation ─────────────────────────────────
...
export default function CompletionCelebration({
  ...
  const [phase, setPhase]             = useState("icon");    // icon → score → breakdown → stats
  ...
  useEffect(() => {
    if (!leveledUp) return;
    const t = setTimeout(() => {
      confetti({ particleCount: 140, spread: 80, origin: { y: 0.5 }, colors: [newLevel.ring, "#ffffff", "#e0e7ff"] });
      setTimeout(() => confetti({ particleCount: 70, spread: 110, origin: { y: 0.35 }, colors: [newLevel.ring, "#fef3c7"] }), 350);
    }, 300);
  ...
  const oldBarPct   = Math.round(((oldScore - oldBandLow) / (oldBandHigh - oldBandLow)) * 100);
  const newBarPct   = Math.round(((newScore - newBandLow) / (newBandHigh - newBandLow)) * 100);
              ...
              <div>
                ...
                <div className="h-2.5 bg-secondary rounded-full overflow-hidden">
                  <motion.div
                    ...
                    animate={{ width: `${Math.min(100, Math.max(0, newBarPct))}%` }}
            ...
            >
              ...
              {nextThresh && !leveledUp && (
                <div
                  ...
                  style={{ borderColor: `${getSpineLevel(nextThresh).ring}40`, backgroundColor: `${getSpineLevel(nextThresh).ring}08` }}
                >
                  <div>
